 ...
 * PROBLEM 1
 *
 * These are the modifcations made to Bitset.java to add the functionality 
 * of determing subsets and cardinality. 
 * 
 *
 ...
 */
    ...
    /**
     *  This method determines the cardinality of _this_ Bitset
     *  by counting the members of a set.  
     *
     *  @return    Integer value of elements
     */
    public int cardinality( )
    {
        int elements = 0;
        
        // Much like our toString method we are checking to see
        // if the value at i is of our set. If it is increment our counter.
        for (int i = 0; i < maxSize; i++) 
        {
            if ( member( i ) ) elements++;
        }
        return elements;
    }

    /**
     *  This method determines if _this_ Bitset is a subset
     *  of _other_Bitset by checking for a null difference.   
     *
     *  @return    Boolean value of subset
     */
    public boolean subset( Bitset setX )
    {
        boolean isSubset = false;
        
        // Get the difference of the two Bitsets
        Bitset diff = difference ( setX );
        
        // If there is no difference, it must be a subset. 
        if ( diff.isNull()  ) {
            isSubset = true;
        }
